...
Gracefully degrades on CPU/MPS (no triton): feature_attention falls back to
PyTorch-native bmm, flash_attn_func raises if called without CuTE-DSL.
"""
...
# ── Fused feature attention ─────────────────────────────────────────────────
#
# Shape: Q, K, V are (B, N_f, D_f) where B=batch*seq_len (~1M), N_f=48, D_f=16.
#
# On CUDA with triton: fused kernel, one program per batch element, everything
# fits in SRAM (~9KB bf16). No intermediate score matrix in global memory.
#
# On CPU/MPS: PyTorch-native bmm fallback (correct, just slower).

HAS_FUSED_FEAT_ATTN = False
...
if HAS_TRITON:
    ...
    HAS_FUSED_FEAT_ATTN = True


def _feat_attn_fallback(q, k, v):
    """PyTorch-native feature attention (softmax). No triton required."""
    scale = k.shape[-1] ** -0.5
...
def feature_attention(q, k, v, activation='softmax'):
    ...
    For softmax on CUDA: fused triton kernel (no global memory for scores).
    For softmax on CPU/MPS: PyTorch-native bmm fallback.
    For silu/silu2: explicit bmm (no fused kernel, all devices).
    ...
    if activation == 'softmax':
        if HAS_FUSED_FEAT_ATTN and q.is_cuda:
            return _feat_attn_fwd_op(q, k, v)
        return _feat_attn_fallback(q, k, v)
    else:
        scale = k.shape[-1] ** -0.5
        scores = torch.bmm(q, k.transpose(-2, -1)) * scale
        if activation == 'silu':
