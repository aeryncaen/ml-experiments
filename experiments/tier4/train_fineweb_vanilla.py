...
class GatedNeighborAttention(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int):
        ...
        self.proj = nn.Linear(d_model, d_model, bias=False)
        self.rotary = Rotary(self.head_dim)
        # Gate projection: input -> single scalar gate per position
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        q = apply_rotary(q, cos, sin)
        k = apply_rotary(k, cos, sin)
        # Content-dependent gate for neighbor accumulation — single scalar per position
        gate = torch.sigmoid(self.gate_proj(x)).unsqueeze(-1)  # (B, T, 1, 1)
