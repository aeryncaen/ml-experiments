...
from heuristic_secrets.joint.model import JointSecretDetector
...
class JointTrainer:
    def __init__(
        self,
        model: JointSecretDetector,
        train_data: list[tuple[torch.Tensor, torch.Tensor, torch.Tensor]],
        val_data: list[tuple[torch.Tensor, torch.Tensor, torch.Tensor]] | None = None,
        lr: float = 1e-3,
    ...
    def train_epoch(self, seed: int | None = None, epoch: int = 0) -> TrainMetrics:
        ...
        for batch_idx in pbar:
            bytes_batch, labels_batch, lengths_batch = self.train_data[batch_idx]
            ...
            out = self.model(bytes_batch, lengths_batch)
            cls_logits = out["cls_logits"]

            loss = self._compute_loss(cls_logits, labels_batch)
            loss.backward()
    ...
    def validate(self, optimize_threshold: bool = False) -> TrainMetrics:
        ...
        for bytes_batch, labels_batch, lengths_batch in self.val_data:
            bytes_batch = bytes_batch.to(self.device)
            labels_batch = labels_batch.to(self.device)
            lengths_batch = lengths_batch.to(self.device)
...
def load_checkpoint(path: Path, device: torch.device | None = None) -> JointSecretDetector:
    from heuristic_secrets.joint.model import JointConfig
    device = device or get_device()
    checkpoint = torch.load(path, map_location=device, weights_only=False)
    config = JointConfig.from_dict(checkpoint["config"])
    model = JointSecretDetector(config)
